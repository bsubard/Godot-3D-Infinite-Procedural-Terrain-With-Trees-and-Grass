...
# --- Sky3D Time of Day Control ---
@export_group("Sky3D Control")
# Drag your Sky3D (WorldEnvironment) node here
...
# --- Underwater Effect (Updated for Sky3D) ---
...
@export var above_water_fog_color: Color = Color(0.7, 0.7, 0.8)
@export var above_water_fog_density: float = 0.0002

# --- Node References (Updated for Sky3D structure) ---
...
# --- Original State Storage (Updated) ---
...
func _ready():
	...
	# Initialize Sky3D reference
	_initialize_sky_node()
	
	# Store original fog settings
	if sky_node and sky_node.environment:
		original_fog_color = sky_node.environment.fog_light_color
		original_fog_density = sky_node.environment.fog_density
	elif world_environment and world_environment.environment:
		...
		print_debug("Warning: No WorldEnvironment found. Underwater effect will be disabled.")
...
func _input(event):
	...
	# Time of day controls (only if Sky3D is available)
	if sky_node:
...
func handle_underwater_effect():
	...
	# Use Sky3D if available, otherwise fallback to world_environment
	var env_node = sky_node if sky_node else world_environment
	
	if env_node and env_node.environment:
		var is_currently_underwater = camera.global_position.y < water_level
		
		# Only update fog if underwater status changed
		if is_currently_underwater != is_underwater:
			
			is_underwater = is_currently_underwater
			#print(env_node.environment.fog_density)
			if is_underwater:
				env_node.environment.fog_light_color = underwater_fog_color
				env_node.environment.fog_density = underwater_fog_density
			else:
				env_node.environment.fog_light_color = above_water_fog_color
				env_node.environment.fog_density = above_water_fog_density

# --- Sky3D Initialization Function ---
func _initialize_sky_node():
	if sky_node_path.is_empty():
		print_debug("Sky Node Path not set. Time of day controls will be disabled.")
		return
	
	var node = get_node_or_null(sky_node_path)
	if not node and get_parent():
		node = get_parent().get_node_or_null(sky_node_path)
	
	if not node:
		print_debug("Could not find Sky node at path: ", sky_node_path)
		return
	elif not node is WorldEnvironment:
		print_debug("Node at Sky Node Path is not a WorldEnvironment.")
		return
	else:
		sky_node = node
		print_debug("Sky3D node initialized successfully.")
